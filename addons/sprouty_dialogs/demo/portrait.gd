...
func _fade_in_animation(node: Node) -> void:
	var end_position = node.position.x
	...
	_tween.set_parallel()
	_tween.tween_property(node, "position:x", end_position, animation_time)
...
func _fade_out_animation(node: Node) -> void:
	var end_position = node.position.x + node.get_viewport().size.x / 5
	_tween = self.create_tween()
